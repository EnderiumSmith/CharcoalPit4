...
public class StillScreen extends AbstractContainerScreen<StillMenu> {
    ...
    public static final ResourceLocation GUI_TEXTURES=ResourceLocation.fromNamespaceAndPath(CharcoalPit.MODID,"textures/gui/container/still.png");

    public StillScreen(StillMenu menu, Inventory playerInventory, Component title) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        FluidStack fluidStack=this.menu.slots.get(this.menu.slots.size()-2).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        drawFluidTooltip(fluidStack,16000,mouseX,mouseY,76,22,24,29,guiGraphics);
        FluidStack fluidStack2=this.menu.slots.get(this.menu.slots.size()-1).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        drawFluidTooltip(fluidStack2,16000,mouseX,mouseY,107,22,16,29,guiGraphics);
    }
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.blit(GUI_TEXTURES, 70, 21, 200, 0, 36, 31);
        guiGraphics.blit(GUI_TEXTURES, 107, 22, 176, 60, 16, 29);
        int time=menu.array.get(0);
        int total=menu.array.get(1);
        if(total>0&&time>=0) {
            int height = (int) (time * 29F / total);
            guiGraphics.blit(GUI_TEXTURES, 124, 22, 188, 31, 9, height);
        }
        ...
        if(burnTime>0) {
            ...
            guiGraphics.blit(GUI_TEXTURES,80,54+12-k,176,12-k,14,k+1);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float v, int i, int i1) {
        ...
        FluidStack fluidStack=this.menu.slots.get(this.menu.slots.size()-2).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack,guiGraphics,leftPos,topPos,72,51,29,16000D);
        renderFluid(fluidStack,guiGraphics,leftPos,topPos,88,51,29,16000D);
        FluidStack fluidStack2=this.menu.slots.get(this.menu.slots.size()-1).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack2,guiGraphics,leftPos,topPos,107,51,29,16000D);
    }
