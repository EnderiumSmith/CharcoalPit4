...
public class TheVatScreen extends AbstractContainerScreen<TheVatMenu> {

    public static final ResourceLocation GUI_TEXTURES=ResourceLocation.fromNamespaceAndPath(CharcoalPit.MODID,"textures/gui/container/the_vat.png");
    private static final int[] BUBBLELENGTHS = new int[]{29, 24, 20, 16, 11, 6, 0};

    public TheVatScreen(TheVatMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.blit(GUI_TEXTURES, 70, 40, 176, 92, 32, 30);
        guiGraphics.blit(GUI_TEXTURES, 116, 17, 176, 0, 16, 28);
        guiGraphics.blit(GUI_TEXTURES, 116, 59, 176, 0, 16, 28);
    }
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        FluidStack fluidStack=this.menu.slots.get(this.menu.slots.size()-3).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        drawFluidTooltip(fluidStack,16000,mouseX,mouseY,76,41,24,28,guiGraphics);
        FluidStack fluidStack2=this.menu.slots.get(this.menu.slots.size()-2).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        drawFluidTooltip(fluidStack2,4000,mouseX,mouseY,116,17,16,28,guiGraphics);
        FluidStack fluidStack3=this.menu.slots.get(this.menu.slots.size()-1).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        drawFluidTooltip(fluidStack3,4000,mouseX,mouseY,116,59,16,28,guiGraphics);
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float v, int i, int i1) {
        ...
        FluidStack fluidStack=this.menu.slots.get(this.menu.slots.size()-3).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack,guiGraphics,leftPos,topPos,70,69,28,16000D);
        renderFluid(fluidStack,guiGraphics,leftPos,topPos,86,69,28,16000D);
        FluidStack fluidStack2=this.menu.slots.get(this.menu.slots.size()-2).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack2,guiGraphics,leftPos,topPos,116,45,28,4000D);
        FluidStack fluidStack3=this.menu.slots.get(this.menu.slots.size()-1).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack3,guiGraphics,leftPos,topPos,116,87,28,4000D);
    }
