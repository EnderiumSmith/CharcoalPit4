...
public class BarrelScreen extends AbstractContainerScreen<BarrelMenu> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawFluidTooltip(fluidStack,16000,mouseX,mouseY,62,14,16,58,guiGraphics);
    }
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.blit(GUI_TEXTURES, 62, 14, 176, 47, 16, 71-14);
        int time=menu.array.get(0);
        int total=menu.array.get(1);
        if(total>0&&time>=0) {
            int height = (int) (time * 14F / total);
            guiGraphics.blit(GUI_TEXTURES, 97, 36, 176, 2, 18, height);
            height = BUBBLELENGTHS[(time) / 2 % 7];
            guiGraphics.blit(GUI_TEXTURES, 82, 13 + height, 176, 17 + height, 12, 28-height);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        FluidStack fluidStack=this.menu.slots.get(this.menu.slots.size()-1).getItem().get(DataComponentRegistry.FLUID_DATA).copy();
        renderFluid(fluidStack,guiGraphics,leftPos,topPos,62,72,58,16000D);
    }
